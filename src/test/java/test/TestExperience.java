...
public class TestExperience implements constants {
    ...
    @Test
    public void Singletest_compare() throws Exception {
        BigInteger x = new BigInteger(String.valueOf(MAXINT));
        BigInteger y = new BigInteger(String.valueOf(MAXINT-1));
        ...
        for (int i = 0; i < 100; i++) {
            boolean result1 = Timecompare(x, y, EncY);
        }
        stopwatch.stop();
//        boolean result2 = Bigcompare(x, y, EncY);
...
//        System.out.println(stopwatch.prettyPrint());
    ...
    public boolean Timecompare(BigInteger x, BigInteger y, BigInteger[] EncY) throws HomomorphicException {
    ...
    public boolean Bigcompare(BigInteger x, BigInteger y, BigInteger[] EncY) throws HomomorphicException {
        //通信量
        ...
        for (int i = 0; i < C.length; i++) {
            C[i] = DGKOperations.multiply(C[i], rnd.nextInt(public_key.getU().intValue()) + 1, public_key);
            communication += RamUsageEstimator.shallowSizeOf(C[i]);
        }
        C = shuffle_bits(C);
//        communication += RamUsageEstimator.shallowSizeOf(C);
        ...
        BigInteger EncDeltaB = DGKOperations.encrypt(deltaB, public_key);
        communication += RamUsageEstimator.shallowSizeOf(EncDeltaB);
        BigInteger delta;
        if (delta_a == 0) {
            delta = EncDeltaB;
        } else {
